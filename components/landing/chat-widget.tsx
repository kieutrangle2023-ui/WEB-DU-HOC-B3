...
export function ChatWidget() {
  ...
  // Nạp lại lịch sử hội thoại của khách từ Supabase (qua route server) khi mở
  // trang — không đọc từ bộ nhớ tạm trên máy, cookie chỉ mang một id vô nghĩa.
  React.useEffect(() => {
    fetch("/api/chat")
      .then((res) => res.json())
  ...
  async function sendMessage(text: string) {
    ...
    try {
      ...
      const reply: string =
        data.reply ??
        "Có lỗi xảy ra, bạn thử lại giúp mình hoặc để lại email/số điện thoại trong form báo giá nhé.";
      ...
      setMessages((prev) => [
        ...
        {
          from: "bot",
          text: "Có lỗi xảy ra, bạn thử lại giúp mình hoặc để lại email/số điện thoại trong form báo giá nhé.",
        },
      ]);
    } finally {
      setLoading(false);
